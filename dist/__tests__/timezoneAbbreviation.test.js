...
describe("Timezone Abbreviation Tests", () => {
    describe("Timezone abbreviation mapping", () => {
        test("should map ET to America/New_York", () => {
            const result = (0, timezone_utils_1.convertTimezoneAbbreviation)("ET");
            expect(result).toBe("America/New_York");
        });
        test("should map EDT to America/New_York", () => {
            const result = (0, timezone_utils_1.convertTimezoneAbbreviation)("EDT");
            expect(result).toBe("America/New_York");
        });
        test("should map EST to America/New_York", () => {
            const result = (0, timezone_utils_1.convertTimezoneAbbreviation)("EST");
            expect(result).toBe("America/New_York");
        });
        test("should map EEST to Europe/Kiev", () => {
            const result = (0, timezone_utils_1.convertTimezoneAbbreviation)("EEST");
            expect(result).toBe("Europe/Kiev");
        });
        test("should map EET to Europe/Kiev", () => {
            const result = (0, timezone_utils_1.convertTimezoneAbbreviation)("EET");
            expect(result).toBe("Europe/Kiev");
        });
    });
    describe("getTimezoneAbbreviation with date parameter", () => {
        test("should return EDT for America/New_York in summer", () => {
            const result = (0, timezone_conversion_service_1.getTimezoneAbbreviation)("America/New_York", summerDate);
            expect(result).toBe("EDT");
            // Verify with moment directly
            const momentResult = moment_timezone_1.default
                .tz(`${summerDate}T12:00:00`, "America/New_York")
                .format("z");
            expect(result).toBe(momentResult);
            // Check DST status
            const isDst = moment_timezone_1.default
                .tz(`${summerDate}T12:00:00`, "America/New_York")
                .isDST();
            expect(isDst).toBe(true);
        });
        test("should return EST for America/New_York in winter", () => {
            const result = (0, timezone_conversion_service_1.getTimezoneAbbreviation)("America/New_York", winterDate);
        ...
        test("should return EEST for Europe/Kyiv in summer", () => {
            const result = (0, timezone_conversion_service_1.getTimezoneAbbreviation)("Europe/Kyiv", summerDate);
            ...
            expect(result).toBe(momentResult);
            // Check DST status
            const isDst = moment_timezone_1.default.tz(`${summerDate}T12:00:00`, "Europe/Kyiv").isDST();
            expect(isDst).toBe(true);
        });
        test("should return EET for Europe/Kyiv in winter", () => {
            const result = (0, timezone_conversion_service_1.getTimezoneAbbreviation)("Europe/Kyiv", winterDate);
            ...
            expect(result).toBe(momentResult);
            // Check DST status
            const isDst = moment_timezone_1.default.tz(`${winterDate}T12:00:00`, "Europe/Kyiv").isDST();
            expect(isDst).toBe(false);
        });
        test("should return JST for Asia/Tokyo (no DST)", () => {
            const result = (0, timezone_conversion_service_1.getTimezoneAbbreviation)("Asia/Tokyo", summerDate);
            expect(result).toBe("JST");
            // Japan doesn't use DST
            const isDst = moment_timezone_1.default.tz(`${summerDate}T12:00:00`, "Asia/Tokyo").isDST();
            expect(isDst).toBe(false);
        });
        test("should return UTC for invalid timezone", () => {
            const result = (0, timezone_conversion_service_1.getTimezoneAbbreviation)("Invalid/Timezone", summerDate);
            expect(result).toBe("UTC");
        });
        test("should return UTC for empty timezone", () => {
            const result = (0, timezone_conversion_service_1.getTimezoneAbbreviation)("", summerDate);
            expect(result).toBe("UTC");
        });
    });
});
