...
import { db } from "../db";
import { slots, preferences, users } from "../db/schema";
import { eq, sql, and, gte, lte, or } from "drizzle-orm";
import { createGoogleClient } from "./google-calendar-utils";
...
import { preserveSourceTimezone } from "./timezone-response-fix";
import {
  processEmailAnalysisWithTimezones,
  getTimezoneAbbreviation,
  convertTimezoneSafe,
  generateShorterSlotsFromRange,
  ...
  return slots.map((slot) => {
    ...
    if (
      ...
      try {
        ...
        const conversion = convertTimezoneSafe(
          date,
          startLocalTime,
          localTz,
          sourceTz
...
// Define a more flexible user type for API endpoints
 ...
 */
function extractSenderInfoFromEmail(emailContent: string): {
  email: string | null;
  name: string | null;
} {
  if (!emailContent) return { email: null, name: null };
 ...
 */
const convertWorkDay = (day: string): string => {
 ...
 */
const parseFlexibleDate = (dateString: string) => {
  ...
  if (!isNaN(parsedDate.getTime())) {
    ...
    if (parsedDate.getFullYear() < currentYear - 5) {
      console.log(
        `Correcting implausible year ${parsedDate.getFullYear()} to current year ${currentYear}`
      );
 ...
 */
const formatForCalendar = (date: Date | string, timezone: string = "UTC") => {
...
const isUserAuthenticated = (req: Request): boolean => {
  ...
  const hasPublicAccess =
    req.publicAccess === true || (req.body && req.body.publicAccess === true);
  ...
  console.log(
    `Authentication check: function=${hasAuthFunction}, authResult=${isAuthViaFunction}, hasUser=${hasUserObject}, hasSessionID=${hasSessionID}, publicAccess=${hasPublicAccess}`
  );
 ...
 */
const handleAnalyzeEmail = async (req: Request, res: Response) => {
  ...
  const isChromeExtension =
    req.headers.origin && req.headers.origin.startsWith("chrome-extension://");
  ...
  let extractedData: {
    ...
    event_title: "Meeting",
    email_reply:
      "Thank you for your email. I'm proposing the following meeting time.",
    meeting_duration: null,
    timezone: null,
  };
  let suggestedSlots: Array<Slot> = [];
  let userPreferences = null;
  let userTimezone = "UTC";
  let extractedTimezone: string | null = null; // Store the timezone extracted from the email
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      model: "gpt-4o",
      messages: [
        { role: "system", content: createSystemPrompt(referenceDate) },
        { role: "user", content: emailContentToAnalyze },
      ],
      temperature: 0.1,
      functions: [dateExtractionSchema],
    ...
    if (functionCall && functionCall.name === "email_analysis") {
      try {
        ...
        console.log(
          "Successfully parsed date/time data:",
        ...
        if (
          ...
          console.log(
            `Processing extracted times with userTimezone: ${userTimezone}`
          );

          // Extract a default timezone from the email if one is commonly used
          let mostCommonTimezone: string | null = null;
          let timezoneCount: Record<string, number> = {};
          ...
          if (mostCommonTimezone) {
            ...
            console.log(
              `Found most common timezone in email: ${extractedTimezone}`
            );
          }
            ...
            if (dateInfo.times && dateInfo.times.length > 0) {
              dateInfo.times.forEach((timeInfo: any) => {
                ...
                if (!timeInfo.sourceTimezone && mostCommonTimezone) {
                  ...
                  console.log(
                    `Applied common timezone ${mostCommonTimezone} to a time slot without timezone`
                  );
                }
                ...
                console.log(
                  `Time slot ${timeInfo.start}-${
                    timeInfo.end
                  } sourceTimezone: ${
                    timeInfo.sourceTimezone || "None (Local Time)"
                  }`
                );
                console.log(
                  `Original extracted times: ${timeInfo.originalStart}-${timeInfo.originalEnd}`
                );
              });
            }
          });
        ...
        extractedData = {
          ...
          event_title: "Meeting",
          email_reply:
            "Thanks for your email. I'd like to schedule a meeting with you.",
          meeting_duration: null,
        };
      }
    } else {
    ...
    const processedResult = processEmailAnalysisWithTimezones(
      extractedData,
      userTimezone,
      extractedData.meeting_duration || null,
      60 // Default slot interval
    );
    ...
    isReplyMode =
      extractedData &&
      extractedData.dates &&
      extractedData.dates.some((d: any) => d.times && d.times.length > 0);

    console.log("Is reply mode:", isReplyMode);
    console.log("Extracted timezone:", extractedTimezone || "Not found");
    console.log("User timezone:", userTimezone);
    console.log(
      "Initial suggested slots:",
      JSON.stringify(suggestedSlots, null, 2)
    );
    ...
    if (suggestedSlots.length > 0) {
      console.log(
        `Starting slot filtering pipeline with ${suggestedSlots.length} initial slots`
      );
      ...
      console.log(
        "Pre-processing slots to ensure consistent flags for time ranges"
      );
      suggestedSlots = suggestedSlots.map((slot: Slot) => {
        ...
        if (slot.isTimeRange === true) {
          ...
          console.log(
            `Fixed flags for time range slot: ${slot.start} - ${slot.end}`
          );
        }
        // For slots already marked as fromTimeRange, make sure isTimeRange is set consistently
          ...
          console.log(
            `Fixed flags for derived slot: ${slot.start} - ${slot.end}`
          );
        }
      ...
      if (isReplyMode) {
        ...
        suggestedSlots = await processSlotsThroughPipeline(
          suggestedSlots,
          user,
          userPreferences,
          userTimezone,
          true, // Enable debug mode to see detailed conflict information
          extractedData.meeting_duration // Pass the meeting duration extracted from email
        );

        console.log(
          `Reply mode: After pipeline filtering: ${suggestedSlots.length} slots available`
        );

        if (suggestedSlots.length === 0) {
          console.log(
            "No slots available after filtering pipeline. Respecting user's preferences and calendar conflicts."
          );
        }
      } else {
        ...
        console.log(
          `Standard mode: After pipeline filtering: ${suggestedSlots.length} slots available`
        );
      }
    }
    ...
    const extractedSenderInfo = extractSenderInfoFromEmail(
      emailContentToAnalyze
    );
    ...
    if (suggestedSlots.length > 0) {
      console.log(
        `Using ${suggestedSlots.length} fully processed slots from timezone-conversion-service.`
      );
      ...
      if (suggestedSlots.length > 0) {
        console.log(
          "Sample slot data:",
          JSON.stringify(suggestedSlots[0], null, 2)
        );
      }
    }

    // Create a recommended slot based on user preferences
    ...
    let recommendedSlot: EnhancedSlot | null = null;
    if (suggestedSlots.length > 0) {
      ...
      let bestSlot: EnhancedSlot = suggestedSlots[0] as EnhancedSlot; // Default to first slot
      ...
      if (timePreference !== "both") {
        ...
        const isAM = (dateStr: string) => {
          // Convert UTC time to user's timezone
          const localHour = moment.tz(dateStr, userTimezone).hour();
          console.log(
            `Slot ${dateStr} is at local hour ${localHour} in ${userTimezone}`
          );
          return localHour < 12;
        };
        ...
        if (preferredSlot) {
          const localTime = moment
            ...
            .format("HH:mm");
          console.log(
            `Using ${timePreference} preferred slot at local time ${localTime} instead of first available slot`
          );
          bestSlot = preferredSlot;
        } else {
          console.log(
            `No ${timePreference} slots available in timezone ${userTimezone}, using first available slot`
          );
        }
      }
      ...
      const isFromTimeRange =
        bestSlot.isTimeRange === true || bestSlot.fromTimeRange === true;
      ...
      if (isFromTimeRange) {
        ...
        startSource =
          bestSlot.start.split("T")[1]?.substring(0, 5) || bestSlot.start;
        endSource = bestSlot.end.split("T")[1]?.substring(0, 5) || bestSlot.end;
        console.log(
          `Using individual slot times (not original time range): ${startSource} - ${endSource}`
        );
      } else {
      ...
      if (slotSourceTimezone && slotSourceTimezone !== localTimeAbbreviation) {
        ...
        startUTC = moment.tz(sourceStartStr, slotSourceTimezone).utc().format();
        endUTC = moment.tz(sourceEndStr, slotSourceTimezone).utc().format();
        ...
        const localStartMoment = moment
          .tz(sourceStartStr, slotSourceTimezone)
          .tz(safeUserTz);
        const localEndMoment = moment
        ...
        console.log(
          `  - Local timezone abbreviation: ${getTimezoneAbbreviation(
            userTimezone
          )}`
        );
      } else {
        ...
        const localStartMoment = moment.tz(bestSlot.start, safeUserTimezone);
        const localEndMoment = moment.tz(bestSlot.end, safeUserTimezone);
        ...
        console.log(`TIMEZONE CONVERSION DEBUG (no source timezone):
          - Original time: ${
            bestSlot.start
          } (assumed to be in ${safeUserTimezone})
          - Formatted local time: ${startLocal}
          - UTC offset of user timezone: ${
            localStartMoment.utcOffset() / 60
          } hours`);

        // Convert from user's local timezone to UTC for storage
        // safeUserTimezone is already defined above
        startUTC = moment.tz(bestSlot.start, safeUserTimezone).utc().format();
        endUTC = moment.tz(bestSlot.end, safeUserTimezone).utc().format();

        console.log(
          `TIMEZONE CONVERSION DETAIL - NO SOURCE TIMEZONE (ASSUMING LOCAL TIME):`
        );
        console.log(`  - User timezone: ${userTimezone}`);
    ...
    const slotsWithLocalTime = suggestedSlots.map((slot) => {
      ...
      if (slot.startUTC) {
        ...
        startDateTime = moment.utc(slot.startUTC).tz(userTimezone);
      } else if (slot.start) {
        ...
        if (typeof slot.start === "string" && slot.start.endsWith("Z")) {
          startDateTime = moment.utc(slot.start).tz(userTimezone);
        } else {
          // Assume it's already in user timezone
          startDateTime = moment.tz(slot.start, userTimezone);
        }
      }

      if (slot.endUTC) {
        // If we have a UTC time, use it as source of truth for conversion
        endDateTime = moment.utc(slot.endUTC).tz(userTimezone);
      } else if (slot.end) {
        ...
        if (typeof slot.end === "string" && slot.end.endsWith("Z")) {
          endDateTime = moment.utc(slot.end).tz(userTimezone);
        } else {
          // Assume it's already in user timezone
          endDateTime = moment.tz(slot.end, userTimezone);
        }
      }

      // Format local times in user's timezone consistently
      const startLocalTime = startDateTime?.format("YYYY-MM-DDTHH:mm:ss");
      const endLocalTime = endDateTime?.format("YYYY-MM-DDTHH:mm:ss");

      // Ensure we have UTC times for storage and APIs
      const startUTCTime =
        startDateTime?.clone().utc().format("YYYY-MM-DDTHH:mm:ss") + "Z";
      const endUTCTime =
        endDateTime?.clone().utc().format("YYYY-MM-DDTHH:mm:ss") + "Z";

      // Debug information
      console.log(
        `[TIMEZONE DEBUG] Processing slot with user timezone: ${userTimezone}`
      );
      console.log(`[TIMEZONE DEBUG] Original slot.start: ${slot.start}`);
      console.log(`[TIMEZONE DEBUG] Original slot.startUTC: ${slot.startUTC}`);
      console.log(
        `[TIMEZONE DEBUG] Converted startLocalTime: ${startLocalTime}`
      );
      ...
      return {
        ...
        localTime: slot.localTime || getTimezoneAbbreviation(userTimezone),
        LocalTime: slot.LocalTime || getTimezoneAbbreviation(userTimezone),
      };
    });
    ...
    const processedExtractedData = preserveSourceTimezone(extractedData);

    // Create a detailed log of the timezone conversion flow for debugging
    console.log(`\n=== TIMEZONE CONVERSION FLOW SUMMARY ===`);
    console.log(
      `User timezone: ${userTimezone} (${getTimezoneAbbreviation(
        userTimezone
      )})`
    );
    console.log(
      `Email timezone (if detected): ${extractedTimezone || "None detected"}`
    );
    ...
    if (recommendedSlot !== null) {
      ...
      console.log(
        `- Start local: ${recommendedSlot.startLocal} (${
          recommendedSlot.localTime || recommendedSlot.LocalTime
        })`
      );
      console.log(
        `- Source timezone: ${
          recommendedSlot.sourceTimezone || "None (local time assumed)"
        }`
      );
    }
    ...
    if (slotsWithLocalTime.length > 0) {
      ...
      console.log(
        `- Start original: ${firstSlot.startSource || firstSlot.start}`
      );
      console.log(`- Start UTC: ${firstSlot.startUTC || "Not available"}`);
      console.log(
        `- Start local: ${firstSlot.startLocal || "Not available"} (${
          firstSlot.localTime
        })`
      );
      console.log(
        `- Source timezone: ${
          firstSlot.sourceTimezone || "None (local time assumed)"
        }`
      );
    }
    ...
    return res.json({
      ...
      extracted: {
        ...
        email_reply:
          processedExtractedData.email_reply || "Thanks for your email.",
      ...
      debug: {
        ...
        initialSlots: convertToLocalSlotType(
          processEmailAnalysisWithTimezones(
            extractedData,
            userTimezone,
            extractedData.meeting_duration || null,
        ...
        afterPastFiltering: filterPastSlots(
          convertToLocalSlotType(
            processEmailAnalysisWithTimezones(
              extractedData,
              userTimezone,
              extractedData.meeting_duration || null,
        ...
        pipeline:
          "Used pipeline-based filtering with proper order for actual slot processing",
        pipelineOrder: [
          "slot expansion",
          "past filtering",
          "working days",
        ...
        currentTime: {
          ...
          utcHoursMinutes: `${new Date().getUTCHours()}:${String(
...
// ==== Slot Filtering Pipeline ====
// These are pure functions for slot filtering, each with a clear input and output
...
type Slot = {
  ...
  // Original values from extracted data
  originalStart?: string | null;
  originalEnd?: string | null;

  // Additional fields for processing
  needsStartTimeCalculation?: boolean;
  needsEndTimeCalculation?: boolean;
...
function filterPastSlots(
  ...
  console.log(
    `[FILTER:PAST] Current time: ${now.toISOString()}, With buffer: ${nowPlusBuffer.toISOString()}`
  );
  ...
  const futureSlotsOnly = slots.filter((slot) => {
    ...
    const slotWithTimezone = slot as {
    ...
    if (slotWithTimezone.sourceTimezone === localTimeAbbr && userTimezone) {
      console.log(
        `[FILTER:PAST:LOCAL] Processing local time slot: ${slot.start} in ${userTimezone}`
      );

      // For local time slots, interpret the time in the user's timezone
      const slotDateInUserTZ = moment.tz(`${slot.start}`, userTimezone);
      const nowInUserTZ = moment.tz(nowPlusBuffer, userTimezone);

      // Compare in the user's timezone for correct results
      const isFuture = slotDateInUserTZ.isAfter(nowInUserTZ);

      console.log(
        `[FILTER:PAST:LOCAL] Slot time in ${userTimezone}: ${slotDateInUserTZ.format(
          "YYYY-MM-DD HH:mm:ss"
        )}`
      );
      console.log(
        `[FILTER:PAST:LOCAL] Current time+buffer in ${userTimezone}: ${nowInUserTZ.format(
          "YYYY-MM-DD HH:mm:ss"
        )}`
      );
      ...
      if (!isFuture) {
        console.log(
          `[FILTER:PAST] Filtered past local time slot: ${slot.start} in ${userTimezone}`
        );
      }
    ...
    if (slot.isTimeRange || slot.isAllDay) {
      ...
      if (isToday) {
        ...
        if (!isFutureTime) {
          console.log(
            `[FILTER:PAST] Filtered today's past time range at ${slotHours}:${slotMinutes}`
          );
        }
        return isFutureTime;
      }
  ...
  console.log(
    `[FILTER:PAST] Filtered out ${
      slots.length - futureSlotsOnly.length
    } past slots, ${futureSlotsOnly.length} remain`
  );
...
function filterByWorkingDays(
  ...
  console.log(
    `[FILTER:WORKDAYS] Filtering by working days: ${workDays.join(
      ","
    )} (${workDays.map((d) => weekdayNames[d]).join(", ")})`
  );
  ...
  const filteredSlots = slots.filter((slot) => {
    try {
      ...
      if (!slot.start) {
        console.warn(
          "[FILTER:WORKDAYS] Slot without start time, skipping filter check:",
          slot
        );
      ...
      if (isNaN(dateObj.getTime())) {
        console.warn(
          "[FILTER:WORKDAYS] Invalid date format in slot, skipping filter check:",
          slot.start
        );
      ...
      console.log(
        `[FILTER:WORKDAYS:DEBUG] Checking slot on ${weekdayName} (${slotDay}): ${slot.start} - isWorkDay: ${isWorkDay}`
      );

      // Filter out non-working days
      if (!isWorkDay) {
        console.log(
          `[FILTER:WORKDAYS] Filtered slot on non-working day ${weekdayName} (${slotDay}): ${slot.start}`
        );
        return false;
      }
  ...
  console.log(
    `[FILTER:WORKDAYS] Filtered out ${
      slots.length - filteredSlots.length
    } slots, ${filteredSlots.length} remain`
  );
...
function filterByWorkingHours(
  ...
  console.log(
    `[FILTER:HOURS] Filtering by working hours: ${workingHoursStart} - ${workingHoursEnd} in timezone: ${
      userTimezone || "UTC"
    }`
  );
  ...
  const filteredSlots = slots.filter((slot) => {
    try {
      ...
      if (slot.isTimeRange) {
        ...
        if (userTimezone) {
          startTime = moment.tz(slot.start, userTimezone);
          endTime = moment.tz(slot.end, userTimezone);
        } else {
          startTime = moment(slot.start);
          endTime = moment(slot.end);
        }
        ...
        const startInWorkingHours =
          startValue >= workStartValue && startValue < workEndValue;
        const endInWorkingHours =
          endValue > workStartValue && endValue <= workEndValue;
        const rangeContainsWorkingHours =
          startValue <= workStartValue && endValue >= workEndValue;

        const overlapsWorkingHours =
          startInWorkingHours || endInWorkingHours || rangeContainsWorkingHours;

        if (!overlapsWorkingHours) {
          console.log(
            `[FILTER:HOURS] Time range ${startTime.format(
              "HH:mm"
            )}-${endTime.format(
              "HH:mm"
            )} doesn't overlap with working hours ${workingHoursStart}-${workingHoursEnd}`
          );
        } else {
          console.log(
            `[FILTER:HOURS] Time range ${startTime.format(
              ...
              "HH:mm"
            )} overlaps with working hours ${workingHoursStart}-${workingHoursEnd}`
          );
        }
      ...
      if (userTimezone) {
        ...
        const localSlotTime = moment.tz(slot.start, userTimezone);
        slotHour = localSlotTime.hour();
        slotMinute = localSlotTime.minute();

        // Debug timezone conversion
        console.log(
          `[FILTER:HOURS:DEBUG] Slot UTC time: ${
            slot.start
          }, Local time in ${userTimezone}: ${localSlotTime.format(
            "YYYY-MM-DD HH:mm"
          )}`
        );
      } else {
      ...
      const isWithinWorkingHours =
        slotValue >= workStartValue && slotValue < workEndValue;

      if (!isWithinWorkingHours) {
        if (userTimezone) {
          const localTime = moment.tz(slot.start, userTimezone).format("HH:mm");
          console.log(
            `[FILTER:HOURS] Filtered slot at local time ${localTime} outside working hours ${workingHoursStart}-${workingHoursEnd}`
          );
        } else {
          console.log(
            `[FILTER:HOURS] Filtered slot outside working hours: ${slot.start}`
          );
        }
        return false;
      }
...
function filterByTimePreference(
  ...
  if (timePreference === "both") {
    console.log(
      `[FILTER:AM/PM] No time preference filtering needed (preference: ${timePreference})`
    );
    return slots;
  }

  console.log(
    `[FILTER:AM/PM] Filtering by time preference: ${timePreference} in timezone: ${
      userTimezone || "UTC"
    }`
  );
  ...
  slots.forEach((slot, index) => {
    if (index < 5 || index > slots.length - 5) {
      ...
      const startTime = userTimezone
        ? moment.tz(slot.start, userTimezone).format("YYYY-MM-DD HH:mm")
        : new Date(slot.start).toISOString();
      const endTime =
        slot.end && userTimezone
          ? moment.tz(slot.end, userTimezone).format("YYYY-MM-DD HH:mm")
          : slot.end
          ? new Date(slot.end).toISOString()
          : "undefined";
      console.log(
        `[FILTER:AM/PM:DEBUG] Slot ${index}: ${startTime} - ${endTime}, isTimeRange: ${!!slot.isTimeRange}, isAllDay: ${!!slot.isAllDay}`
      );
    }
    if (index === 5 && slots.length > 10) {
      console.log(
        `[FILTER:AM/PM:DEBUG] ... ${slots.length - 10} more slots ...`
      );
    }
  });
  ...
  const filteredSlots = slots.filter((slot) => {
    try {
      ...
      if (userTimezone) {
        // Convert UTC time to user's timezone
        slotHour = moment.tz(slot.start, userTimezone).hour();
      ...
      console.log(
        `[FILTER:AM/PM:DEBUG] Processing slot: ${startLocalTime}${
          slot.end ? ` - ${endLocalTime}` : ""
        }, slotHour: ${slotHour}, isPM: ${slotHour >= 12}`
      );
      console.log(
        `[FILTER:AM/PM:DEBUG] Slot flags: isTimeRange=${!!slot.isTimeRange}, isAllDay=${!!slot.isAllDay}, isSpecificTime=${!!slot.isSpecificTime}`
      );
      ...
      if (
        ...
        console.log(
          `[FILTER:AM/PM:DEBUG] Processing as time range, all-day or adapted duration slot`
        );
        ...
        if (timePreference === "pm") {
          ...
          if (userTimezone && slot.end) {
            endHour = moment.tz(slot.end, userTimezone).hour();
          } else if (slot.end) {
          ...
          console.log(
            `[FILTER:AM/PM:DEBUG] Time range with startHour: ${slotHour}, endHour: ${endHour}, startIsPM: ${startIsPM}, endIsPM: ${endIsPM}, spansPM: ${spansPM}`
          );

          if (!spansPM) {
            console.log(
              `[FILTER:AM/PM] Filtered range slot without PM times: ${startLocalTime} (startHour: ${slotHour}, endHour: ${endHour})`
            );
            return false;
          }

          console.log(
            `[FILTER:AM/PM:DEBUG] Keeping range slot that includes PM time: ${startLocalTime}`
          );
          return true;
          ...
          let startHour;
          if (userTimezone) {
            startHour = moment.tz(slot.start, userTimezone).hour();
          } else {
          ...
          console.log(
            `[FILTER:AM/PM:DEBUG] Time range with startHour: ${startHour}, endHour: ${endHour}, startIsAM: ${startIsAM}, endIsAM: ${endIsAM}, spansAM: ${spansAM}`
          );

          if (!spansAM) {
            console.log(
              `[FILTER:AM/PM] Filtered range slot without AM times: ${startLocalTime} (startHour: ${startHour}, endHour: ${endHour})`
            );
            return false;
          }

          console.log(
            `[FILTER:AM/PM:DEBUG] Keeping range slot that includes AM time: ${startLocalTime}`
          );
          return true;
        }
      }
      // For specific time slots
        ...
        const matchesPreference =
          (timePreference === "pm" && isPM) ||
          (timePreference === "am" && !isPM);

        console.log(
          `[FILTER:AM/PM:DEBUG] Specific time slot check: slotHour=${slotHour}, isPM=${isPM}, timePreference=${timePreference}, matches=${matchesPreference}`
        );
        ...
        if (!matchesPreference) {
          if (userTimezone) {
            const localTime = moment
              ...
              .format("HH:mm");
            console.log(
              `[FILTER:AM/PM] Filtered slot at local time ${localTime} not matching ${timePreference} preference`
            );
          } else {
            console.log(
              `[FILTER:AM/PM] Filtered slot not matching ${timePreference} preference: ${slot.start}`
            );
          }
          return false;
        }
        ...
        console.log(
          `[FILTER:AM/PM:DEBUG] Keeping specific time slot that matches preference: ${startLocalTime}`
        );
      }
  ...
  console.log(
    `[FILTER:AM/PM] Summary: ${slots.length} input → ${
      filteredSlots.length
    } output, filtered out ${slots.length - filteredSlots.length} slots`
  );
 ...
 */
function filterByBufferAlignment(
  slots: Array<Slot>,
  ...
  console.log(
    `[FILTER:BUFFER] Applying buffer alignment filter with ${bufferMinutes} minutes buffer in timezone ${
      userTimezone || "not provided"
    }`
  );
  ...
  const slotsByDate: Record<
    string,
    Array<{ slot: Slot; start: moment.Moment; end: moment.Moment }>
  > = {};
  ...
  slots.forEach((slot) => {
    try {
      ...
      const startMoment = userTimezone
        ? moment.tz(slot.start, userTimezone)
        : moment(slot.start);
      const endMoment = userTimezone
        ? moment.tz(slot.end, userTimezone)
        : moment(slot.end);
  ...
  Object.keys(slotsByDate).forEach((date) => {
    ...
    dateSlots.forEach((current, index) => {
      ...
      if (hasBufferBefore && hasBufferAfter) {
        ...
        console.log(
          `[FILTER:BUFFER] Filtered out slot at ${current.start.format(
            "HH:mm"
          )} due to insufficient buffer ${hasBufferBefore ? "" : "before"} ${
            hasBufferAfter ? "" : "after"
          }`
        );
      }
    });
  ...
  console.log(
    `[FILTER:BUFFER] Filtered out ${
      slots.length - filteredSlots.length
    } slots without proper buffer, ${filteredSlots.length} remain`
  );
...
async function filterByCalendarAvailability(
  ...
  workingHoursStart: string,
  workingHoursEnd: string,
  debugMode: boolean = true // Always enable debug mode to help with diagnostics
): Promise<Array<Slot>> {
  console.log(
    `[FILTER:CALENDAR] Checking calendar availability for ${slots.length} slots`
  );
    ...
    try {
      ...
      if (slot.isTimeRange || slot.isAllDay || slot.fromTimeRange) {
        ...
        if (slot.isAllDay) {
          ...
          const rangeStart = moment.tz(`${startTimeStr}`, userTimezone);
          const rangeEnd = moment.tz(`${endTimeStr}`, userTimezone);
          ...
          while (
            ...
            try {
              ...
              if (busyPeriods.length === 0) {
                ...
                const slotRangeLocal = `${currentStart.format(
                  "YYYY-MM-DD HH:mm"
                )} - ${slotEnd.format("YYYY-MM-DD HH:mm")}`;
                ...
                for (const busy of busyPeriods) {
                  ...
                  const isRealConflict = !(
                    busyEnd.isSameOrBefore(currentStart) ||
                    busyStart.isSameOrAfter(slotEnd)
                  );

                  if (isRealConflict) {
                    console.log(
                      `[FILTER:CALENDAR:ALL-DAY:CONFLICT] This is a real conflict with the actual meeting time`
                    );
                    hasActualConflict = true;
                  } else {
                    // This is only a buffer conflict, not a real meeting conflict
                    console.log(
                      `[FILTER:CALENDAR:ALL-DAY:CONFLICT] This is only a buffer overlap, not a real meeting conflict!`
                    );

                    // Determine if it's a start buffer or end buffer overlap
                    if (busyEnd.isSameOrBefore(currentStart)) {
                      console.log(
                        `[FILTER:CALENDAR:ALL-DAY:CONFLICT] Conflict only with start buffer time`
                      );
                    } else {
                      console.log(
                        `[FILTER:CALENDAR:ALL-DAY:CONFLICT] Conflict only with end buffer time`
                      );
                    }
                  }
                }
                ...
                if (hasActualConflict) {
                  console.log(
                    `[FILTER:CALENDAR:ALL-DAY:REJECT] Rejected all-day slot with real calendar conflicts`
                  );
                } else {
                  // Add the slot if there are only buffer conflicts
                  console.log(
                    `[FILTER:CALENDAR:ALL-DAY:ACCEPT] Accepted all-day slot despite buffer conflicts`
                  );
                  generatedSlots.push({
                    start: currentStart.utc().toISOString(),
                    end: slotEnd.utc().toISOString(),
              ...
              console.error(
                "[FILTER:CALENDAR] Error checking calendar for all-day slot:",
                error
              );
            }
          ...
          console.log(
            `[FILTER:CALENDAR] Generated ${generatedSlots.length} available slots for all-day slot`
          );
          ...
          const startTimeHours = parseInt(
            startTimeStr.split("T")[1].split(":")[0]
          );
          const startTimeMinutes = parseInt(
            startTimeStr.split("T")[1].split(":")[1]
          );
          const endTimeHours = parseInt(endTimeStr.split("T")[1].split(":")[0]);
          const endTimeMinutes = parseInt(
            endTimeStr.split("T")[1].split(":")[1]
          );

          const workStartHours = parseInt(
            workStartStr.split("T")[1].split(":")[0]
          );
          const workStartMinutes = parseInt(
            workStartStr.split("T")[1].split(":")[1]
          );
          const workEndHours = parseInt(workEndStr.split("T")[1].split(":")[0]);
          const workEndMinutes = parseInt(
            workEndStr.split("T")[1].split(":")[1]
          );
          ...
          const adjustedStartTimeStr = `${dateStr}T${String(
            adjustedStartHours
          ).padStart(2, "0")}:${String(adjustedStartMinutes).padStart(
            2,
            "0"
          )}:00`;
          const adjustedEndTimeStr = `${dateStr}T${String(
            adjustedEndHours
          ).padStart(2, "0")}:${String(adjustedEndMinutes).padStart(
            2,
            "0"
          )}:00`;
          ...
          const adjustedStartValue =
            adjustedStartHours * 60 + adjustedStartMinutes;
          const adjustedEndValue = adjustedEndHours * 60 + adjustedEndMinutes;

          if (adjustedStartValue >= adjustedEndValue) {
            console.log(
              "[FILTER:CALENDAR] Skipping invalid time range (start >= end)"
            );
            continue;
          }

          // Generate slots
          const rangeStart = moment.tz(`${adjustedStartTimeStr}`, userTimezone);
          const rangeEnd = moment.tz(`${adjustedEndTimeStr}`, userTimezone);
            ...
            try {
              ...
              if (busyPeriods.length === 0) {
                ...
                for (const busy of busyPeriods) {
                  ...
                  if (isRealConflict) {
                    console.log(
                      `[FILTER:CALENDAR:TIME-RANGE:CONFLICT] This is a real conflict with the actual meeting time`
                    );
                    hasActualConflict = true;
                  } else {
                    // This is only a buffer conflict, not a real meeting conflict
                    console.log(
                      `[FILTER:CALENDAR:TIME-RANGE:CONFLICT] This is only a buffer overlap, not a real meeting conflict!`
                    );

                    // Determine if it's a start buffer or end buffer overlap
                    if (busyEnd.isSameOrBefore(currentStart)) {
                      console.log(
                        `[FILTER:CALENDAR:TIME-RANGE:CONFLICT] Conflict only with start buffer time`
                      );
                    } else {
                      console.log(
                        `[FILTER:CALENDAR:TIME-RANGE:CONFLICT] Conflict only with end buffer time`
                      );
                    }
                  }
                }
                ...
                if (hasActualConflict) {
                  console.log(
                    `[FILTER:CALENDAR:TIME-RANGE:REJECT] Rejected time range slot with real calendar conflicts`
                  );
                } else {
                  // Add the slot if there are only buffer conflicts
                  console.log(
                    `[FILTER:CALENDAR:TIME-RANGE:ACCEPT] Accepted time range slot despite buffer conflicts`
                  );
                  generatedSlots.push({
                    start: currentStart.utc().toISOString(),
                    end: slotEnd.utc().toISOString(),
              ...
              console.error(
                "[FILTER:CALENDAR] Error checking calendar for time range slot:",
                error
              );
            }
          ...
          console.log(
            `[FILTER:CALENDAR] Generated ${generatedSlots.length} available slots for time range`
          );
          availableSlots.push(...generatedSlots);
        }
      } else {
        ...
        try {
          ...
          console.log(
            `[FILTER:CALENDAR:SPECIFIC] Checking specific time slot: ${slotStart.format(
              "YYYY-MM-DD HH:mm"
            )} - ${slotEnd.format("YYYY-MM-DD HH:mm")}`
          );
          console.log(
            `[FILTER:CALENDAR:SPECIFIC] Including buffer: ${queryStart.format(
              "YYYY-MM-DD HH:mm"
            )} - ${queryEnd.format("YYYY-MM-DD HH:mm")}`
          );
          console.log(
            `[FILTER:CALENDAR:SPECIFIC] User timezone: ${userTimezone}`
          );

          if (slot.fromTimeRange) {
            console.log(
              `[FILTER:CALENDAR:SPECIFIC] This slot was generated from a time range`
            );
          }
          ...
          console.log(
            `[FILTER:CALENDAR:SPECIFIC] API response: ${JSON.stringify(
          ...
          console.log(
            `[FILTER:CALENDAR:SPECIFIC] Found ${busyPeriods.length} busy periods`
          );

          if (busyPeriods.length === 0) {
            console.log(
              `[FILTER:CALENDAR:SPECIFIC:SUCCESS] Slot is available and will be added: ${slotStart.format(
                "YYYY-MM-DD HH:mm"
              )} - ${slotEnd.format("YYYY-MM-DD HH:mm")}`
            );
            ...
            for (const busy of busyPeriods) {
              ...
              console.log(
                `[FILTER:CALENDAR:SPECIFIC:CONFLICT] Conflict detected in timezone ${userTimezone}:`
              );
              console.log(
                `[FILTER:CALENDAR:SPECIFIC:CONFLICT] Slot: ${slotRangeLocal}`
              );
              console.log(
                `[FILTER:CALENDAR:SPECIFIC:CONFLICT] Busy: ${busyRangeLocal}`
              );
              console.log(
                `[FILTER:CALENDAR:SPECIFIC:CONFLICT] Raw busy data: ${JSON.stringify(
                  busy
                )}`
              );
              ...
              const isRealConflict = !(
                busyEnd.isSameOrBefore(slotStart) ||
                busyStart.isSameOrAfter(slotEnd)
              );

              if (isRealConflict) {
                console.log(
                  `[FILTER:CALENDAR:SPECIFIC:CONFLICT] This is a real conflict with the actual meeting time`
                );
                hasActualConflict = true;
              } else {
                // This is only a buffer conflict, not a real meeting conflict
                console.log(
                  `[FILTER:CALENDAR:SPECIFIC:CONFLICT] This is only a buffer overlap, not a real meeting conflict!`
                );

                // Determine if it's a start buffer or end buffer overlap
                if (busyEnd.isSameOrBefore(slotStart)) {
                  console.log(
                    `[FILTER:CALENDAR:SPECIFIC:CONFLICT] Conflict only with start buffer time`
                  );
                } else {
                  console.log(
                    `[FILTER:CALENDAR:SPECIFIC:CONFLICT] Conflict only with end buffer time`
                  );
                }
              }
            }
            ...
            if (hasActualConflict) {
              console.log(
                `[FILTER:CALENDAR:SPECIFIC:REJECT] Slot rejected due to real calendar conflicts: ${slotStart.format(
                  "YYYY-MM-DD HH:mm"
                )} - ${slotEnd.format("YYYY-MM-DD HH:mm")}`
              );
            } else {
              console.log(
                `[FILTER:CALENDAR:SPECIFIC:ACCEPT] Slot accepted despite buffer conflicts: ${slotStart.format(
                  "YYYY-MM-DD HH:mm"
                )} - ${slotEnd.format("YYYY-MM-DD HH:mm")}`
              );
              ...
              console.log(
                `[FILTER:CALENDAR:BUFFER-FIX] Adding slot with buffer-only conflicts`
              );
              availableSlots.push(slot);
            }
          }
          ...
          console.error(
            "[FILTER:CALENDAR] Error checking calendar for specific slot:",
            error
          );
        }
      }
  ...
  console.log(
    `[FILTER:CALENDAR] After availability check: ${availableSlots.length} slots available`
  );
 ...
 */
async function processSlotsThroughPipeline(
  slots: Array<Slot>,
  user: AuthenticatedUser,
  userPreferences: any,
  userTimezone: string,
  debug: boolean = false, // Add debug flag
  extractedMeetingDuration: number | null = null // Add parameter for extracted meeting duration
): Promise<Array<Slot>> {
  ...
  const meetingDuration =
    extractedMeetingDuration || userPreferences.meetingDuration || 30;
  console.log(
    `Meeting duration: ${meetingDuration} minutes ${
      extractedMeetingDuration ? "(from email)" : "(from user preferences)"
    }`
  );
  ...
  console.log(
    `[PIPELINE:PARAMS] Working hours: ${workingHoursStart} - ${workingHoursEnd}`
  );
  ...
  processedSlots.forEach((slot, index) => {
    console.log(
      `[PIPELINE:SLOT:${index}] ${slot.start} - ${slot.end} (isTimeRange: ${slot.isTimeRange}, isAllDay: ${slot.isAllDay})`
    );
  });
  ...
  console.log(
    `[PIPELINE:STEP1] Starting slot expansion: Processing ${processedSlots.length} initial slots`
  );

  let expandedSlots: Slot[] = [];
  for (const slot of processedSlots) {
    ...
    if (slot.needsEndTimeCalculation === true) {
      console.log(
        `[PIPELINE:SLOT:EXPAND] Detected slot with open-ended time range: ${slot.start}`
      );
      ...
      console.log(
        `[PIPELINE:SLOT:EXPAND] Applied working hours end time: ${slot.end}`
      );
    ...
    if (slot.start && slot.end) {
      ...
      if (!slot.sourceTimezone && userTimezone) {
        ...
        start = moment.tz(`${startDate}T${startTime}`, userTimezone);
        end = moment.tz(`${endDate}T${endTime}`, userTimezone);
      } else {
      ...
      console.log(
        `[PIPELINE:SLOT:EXPAND] Processing slot ${slot.start} - ${slot.end}`
      );
      console.log(
        `[PIPELINE:SLOT:EXPAND] Duration: ${durationMinutes} min, Meeting duration: ${meetingDuration} min`
      );
      console.log(
        `[PIPELINE:SLOT:EXPAND] Flags: isTimeRange=${slot.isTimeRange}, isAllDay=${slot.isAllDay}, fromTimeRange=${slot.fromTimeRange}, fromAllDay=${slot.fromAllDay}`
      );
      ...
      if (durationMinutes < meetingDuration) {
        ...
        if (durationMinutes >= 25) {
          console.log(
            `[PIPELINE:ADAPT] Adapting meeting duration from ${meetingDuration} to ${durationMinutes} minutes to fit the shorter time range`
          );
      ...
      if (
        durationMinutes > adaptedMeetingDuration ||
        slot.isTimeRange ||
        slot.isAllDay ||
        slot.fromTimeRange ||
        slot.fromAllDay ||
        ...
        const shorterSlots = generateShorterSlotsFromRange(
          slot,
          adaptedMeetingDuration, // Using adaptedMeetingDuration which respects email duration
          slotInterval,
          userTimezone
        );

        console.log(
          `[PIPELINE:SLOT:EXPAND] Generated ${shorterSlots.length} shorter slots`
        );
        // Log first few generated slots for verification
        shorterSlots.slice(0, 3).forEach((shorter, idx) => {
          console.log(
            `[PIPELINE:SLOT:EXPAND:GENERATED:${idx}] ${shorter.start} - ${shorter.end}`
          );
        });
        if (shorterSlots.length > 3) {
          console.log(
            `[PIPELINE:SLOT:EXPAND:GENERATED] ... and ${
              shorterSlots.length - 3
            } more slots`
          );
        }
        ...
        console.log(
          `[PIPELINE:SLOT:EXPAND] Keeping original slot (no expansion needed)`
        );
      ...
      console.log(
        `[PIPELINE:SLOT:EXPAND] Slot has incomplete time info, keeping as is`
      );
      expandedSlots.push(slot);
    }
  }
  ...
  console.log(
    `[PIPELINE:STEP1] Slot expansion results: ${slots.length} original slots -> ${processedSlots.length} expanded slots`
  );
  ...
  console.log(
    `[PIPELINE:STEP2] Past filtering: ${slotsBeforePastFilter} -> ${processedSlots.length}`
  );
  ...
  console.log(
    `[PIPELINE:STEP4] Working hours filter: ${slotsBeforeWorkHoursFilter} -> ${processedSlots.length}`
  );

  // Step 5: Filter by time preference (AM/PM) with user's timezone
  const slotsBeforeTimePreferenceFilter = processedSlots.length;
  processedSlots = filterByTimePreference(
    processedSlots,
    timePreference,
    userTimezone
  );
  console.log(
    `[PIPELINE:STEP5] Time preference filter (${timePreference}): ${slotsBeforeTimePreferenceFilter} -> ${processedSlots.length}`
  );
  ...
  processedSlots = filterByBufferAlignment(
    processedSlots,
    bufferMinutes,
    userTimezone
  );
  console.log(
    `[PIPELINE:STEP6] Buffer alignment filter: ${slotsBeforeBufferFilter} -> ${processedSlots.length}`
  );

  // Step 7: Filter by calendar availability with buffer time applied
  const slotsBeforeCalendarFilter = processedSlots.length;
  processedSlots = await filterByCalendarAvailability(
    processedSlots,
    user,
    bufferMinutes,
    userTimezone,
    ...
    workingHoursStart,
    workingHoursEnd,
    debug // Pass the debug flag to see more detailed conflict info
  );
  console.log(
    `[PIPELINE:STEP7] Calendar availability filter: ${slotsBeforeCalendarFilter} -> ${processedSlots.length}`
  );

  // Final summary of pipeline stages
  console.log(`[PIPELINE:SUMMARY] Filtering stages:
    Initial: ${slots.length}
    After past filtering: ${slotsBeforePastFilter} -> ${
    processedSlots.length !== expandedSlots.length
    ...
    After calendar check: ${slotsBeforeCalendarFilter} -> ${
    processedSlots.length
  }
  `);

  console.log(
    `Slot filtering pipeline complete: ${processedSlots.length} slots remaining`
  );
 ...
 * @param res Express response
 */
const handleCreateSlot = async (req: Request, res: Response) => {
 ...
 * @param res Express response
 */
const handleSlotConfirmation = async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  const eventTitle =
    req.method === "POST" ? req.body.eventTitle : req.query.eventTitle;
  ...
  try {
    ...
    if (existingSlot) {
      return await handleExistingSlotConfirmation(
        existingSlot,
        email as string,
        attendeeName,
    ...
    return await handleSimulatedSlotConfirmation(
      id,
      email as string,
      attendeeName,
      eventTitle as string,
      res,
      req // Pass the req object to the function
    );
  } catch (error) {
 ...
 */
const handleExistingSlotConfirmation = async (
  existingSlot: any,
  email: string,
  attendeeName: string,
  eventTitle: string,
  ...
  await db
    ...
    .where(eq(slots.id, existingSlot.id));
  ...
  try {
    const [owner] = await db
      .select()
      .from(users)
      .where(eq(users.id, existingSlot.userId));

    if (owner && owner.accessToken) {
      const auth = createGoogleClient(owner as any);
      ...
      console.log(
        `Adding magic wand emoji to event title for slot from calendar link`
      );
      eventTitle = `🪄 ${eventTitle || "Meeting via Slot Link"}`;
 ...
 */
const handleSimulatedSlotConfirmation = async (
  id: string,
  email: string,
  attendeeName: string,
  eventTitle: string,
  res: Response,
  req?: Request // Add req as an optional parameter
) => {
  const calendar = google.calendar("v3");
  let dateStr: string;
  let timeStr: string;
  ...
  const timeFormat = req?.body?.timeFormat || "ORIGINAL"; // Default to original if not specified
  const providedTimezone = req?.body?.timezone; // Timezone provided by the client
  ...
  let providedStartTime = req?.body?.startTime
    ? new Date(req.body.startTime)
    : null;
  let providedEndTime = req?.body?.endTime ? new Date(req.body.endTime) : null;
  ...
  if (areTimesValid) {
    console.log("Using provided start and end times:", {
      startTime: providedStartTime?.toISOString(),
      endTime: providedEndTime?.toISOString(),
      timeFormat,
      timezone: userTimezone,
      duration:
        providedEndTime && providedStartTime
  ...
  // Handle nanoid-based IDs (from reply mode)
    ...
    const [existingLinkSlot] = await db
      .select()
      .from(slots)
      .where(eq(slots.linkId, id))
      .limit(1);
    ...
    if (existingLinkSlot) {
      ...
      dateStr = moment(existingLinkSlot.start).format("YYYY-MM-DD");
      timeStr = moment(existingLinkSlot.start).format("HH:mm:ss");

      // Check if this slot has a stored meeting duration (from email extraction)
      if (existingLinkSlot.duration) {
        console.log(
          `Using extracted meeting duration from slot: ${existingLinkSlot.duration} minutes`
        );
      ...
      if (startTime && endTime) {
        ...
        if (!isNaN(providedStart.getTime())) {
          dateStr = moment(providedStart).format("YYYY-MM-DD");
          timeStr = moment(providedStart).format("HH:mm:ss");
        ...
        console.log(
          "Using tomorrow same time for nanoid slot (no time data provided):",
          {
            id,
            dateStr,
            timeStr,
  ...
  if (targetUserId) {
    ...
    console.log(
      `Using specified user ID ${targetUserId} from prefixed link:`,
      !!targetUser
    );
  }
  ...
  if (!targetUser) {
    ...
    if (req?.user) {
      ...
      console.log("Using authenticated user from request:", {
        ...
        hasAccessToken: !!targetUser?.accessToken,
        hasRefreshToken: !!targetUser?.refreshToken,
      });
    }
    ...
    if (!targetUser) {
      ...
      [targetUser] = await db
        ...
        .from(users)
        .where(sql`${users.email} != 'olena@h34rt.ai'`)
        .limit(1);
  ...
  if (areTimesValid && providedStartTime && providedEndTime) {
    ...
    if (timeFormat === "UTC") {
      ...
      startTime = moment.utc(providedStartTime).tz(userTimezone).toDate();
      endTime = moment.utc(providedEndTime).tz(userTimezone).toDate();
      ...
      startTime = moment
        .tz(
          moment(providedStartTime).format("YYYY-MM-DDTHH:mm:ss"),
          userTimezone
        )
        .toDate();
      endTime = moment
        .tz(moment(providedEndTime).format("YYYY-MM-DDTHH:mm:ss"), userTimezone)
        .toDate();
    } else {
      // For ORIGINAL or other formats, use as provided but ensure timezone
      console.log(
        `Processing times in format ${timeFormat} with timezone ${userTimezone}`
      );
    ...
    startTime = moment.tz(dateStr + "T" + timeStr, userTimezone).toDate();
    ...
    if (req && req.body && req.body.extractedMeetingDuration) {
      ...
      console.log(
        `Using meeting duration from email extraction: ${meetingDuration} minutes`
      );
    } else {
      ...
      try {
        ...
        if (targetUser?.id) {
          const [userPrefs] = await db
            .select()
            .from(preferences)
            .where(eq(preferences.userId, targetUser.id));

          if (userPrefs?.meetingDuration) {
            meetingDuration = userPrefs.meetingDuration;
            console.log(
              `Using user's preferred meeting duration: ${meetingDuration} minutes`
            );
          }
        }
      } catch (prefsError) {
    ...
    endTime = moment(startTime).add(meetingDuration, "minutes").toDate();
    console.log(
      `Using calculated end time with ${meetingDuration} minutes duration`
    );
  }
  ...
  try {
    ...
    if (targetUser.accessToken) {
      try {
        ...
        // This is for direct link confirmations like https://app.meetalphie.com/calendar/slot/[slotID]
        console.log(
          `Adding magic wand emoji to event title for slot ${newSlotId} from calendar link`
        );
        eventTitle = `🪄 ${eventTitle || "Meeting via Slot Link"}`;

        const auth = createGoogleClient(targetUser as any);
        // Always preserve local time for consistent behavior
        let startDateTime, endDateTime;

        // For all slots, preserve local time exactly in the user's timezone
        console.log(
          `Preserving local time in ${userTimezone} timezone for calendar event`
        );
        ...
        console.log(
          `Processing times with format ${timeFormat} for calendar event`
        );
        ...
        if (
          ...
          console.log(
            "Preserving UTC times for calendar event while ensuring proper format"
          );

          // First parse the times in UTC
          startMoment = moment.utc(req.body.startTime);
          endMoment = moment.utc(req.body.endTime);
          ...
          startMoment = moment.tz(startTime, userTimezone);
          endMoment = moment.tz(endTime, userTimezone);
        ...
        console.log(
          `Local time preserved: ${startDateTime} - ${endDateTime} in ${userTimezone}`
        );

        // Additional timezone debugging for event creation
        console.log("TIMEZONE DEBUG - Calendar event creation:", {
          timeFormat,
          startTimeFromRequest: req?.body?.startTime,
          endTimeFromRequest: req?.body?.endTime,
          providedTimezone: req?.body?.timezone,
          userTimezone,
          startTime: startTime.toISOString(),
          endTime: endTime.toISOString(),
        ...
        await db.insert(slots).values({
          ...
          duration: req?.body?.duration || null, // Include extracted meeting duration if available
        });
    ...
    try {
      const { queueCalendarEventCreation } = await import(
        "./calendar-background-service"
      );
    ...
    return res.json({
      ...
      calendarCreated: false,
      message:
        "Slot created successfully. Calendar event will be created shortly.",
      slotId: newSlotId,
      originalId: id,
      event: {
        start: startTime.toISOString(),
 ...
 * @param res Express response
 */
const handleGetSlotDetails = async (req: Request, res: Response) => {
  const { id } = req.params;
  const calendar = google.calendar("v3");
  ...
  try {
    ...
    let slot: any = null;
    let pureDateId: string | null = null;
    let userId: number | null = null;

    // First try to find by exact ID
    const [exactIdSlot] = await db
      .select({
        id: slots.id,
        userId: slots.userId,
        start: slots.start,
        ...
        attendeeEmail: slots.attendeeEmail,
        attendeeName: slots.attendeeName,
        eventTitle: slots.eventTitle,
        eventId: slots.eventId,
        calendarCreated: slots.calendarCreated,
        linkId: slots.linkId,
        duration: slots.duration, // Include the duration field
      })
      .from(slots)
      .where(eq(slots.id, id));
    ...
    if (exactIdSlot) {
      ...
      const [matchingSlot] = await db
        ...
        .from(slots)
        .where(or(eq(slots.simulatedId, id), eq(slots.linkId, id)));
      ...
      if (matchingSlot) {
        ...
        if (userPrefixMatch) {
          ...
          try {
            ...
            if (req?.user) {
              ...
              if (authUser?.timezone) {
                ...
                console.log(
                  `Using authenticated user's timezone: ${userTimezone}`
                );
              }
            } else {
              ...
              if (!anyUser) {
                ...
                if (fallbackUser?.timezone) {
                  ...
                  console.log(
                    `Using fallback user's timezone: ${userTimezone}`
                  );
                }
              } else if (anyUser?.timezone) {
          ...
          const startTime = moment.tz(localDateStr, userTimezone).toDate();
          ...
          try {
            ...
            // Final fallback to any user
            if (!defaultUser) {
              [defaultUser] = await db.select().from(users).limit(1);
            }
            ...
            if (defaultUser) {
              ...
              const [userPrefs] = await db
                .select()
                .from(preferences)
                .where(eq(preferences.userId, defaultUser.id));

              if (userPrefs?.meetingDuration) {
                meetingDuration = userPrefs.meetingDuration;
                console.log(
                  `Using default user's preferred meeting duration: ${meetingDuration} minutes`
                );
              }
            }
          } catch (prefsError) {
    ...
    if (slot.userId) {
      try {
        ...
        const [userPrefs] = await db
          .select()
          .from(preferences)
          .where(eq(preferences.userId, slot.userId));
      ...
      if (slot.confirmed && slot.eventId) {
        console.log("Checking calendar status for confirmed slot:", {
          ...
          hasUserTokens: !!userWithTokens?.accessToken,
        });
        ...
        try {
          ...
          if (userWithTokens && userWithTokens.accessToken) {
            ...
            const userForAuth: AuthenticatedUser = {
              ...
              accessToken: userWithTokens.accessToken as string, // We've already checked it exists above
              refreshToken: userWithTokens.refreshToken || null,
              timezone: userWithTokens.timezone || null,
            };
            const auth = createGoogleClient(userForAuth);

            try {
              console.log(
                `Checking Google Calendar for event: ${slot.eventId}`
              );
              ...
              console.log(
                `Calendar event status for ${slot.eventId}:`,
                eventResponse.data.status
              );

              // Check if the event has been cancelled or deleted
              if (eventResponse.data.status === "cancelled") {
                console.log(
                  `Event ${slot.eventId} for slot ${slot.id} has been cancelled`
                );
              ...
              if (
                ...
                console.log(
                  `Event ${slot.eventId} for slot ${slot.id} no longer exists in Google Calendar`
                );
                verifiedCalendarStatus = false;
                ...
                console.error(
                  `Error checking calendar event status for slot ${slot.id}:`,
                  calendarError
                );
              }
            }
            ...
            console.log(
              `Skipping Google Calendar check for slot ${slot.id} - no valid user tokens available`
            );
          }
        } catch (err) {
          console.warn(
            `Error verifying Google Calendar event for slot ${slot.id}:`,
            err
          );
        ...
        console.log(
          `Slot ${slot.id} is not eligible for calendar verification:`,
          {
    ...
    const isAutonomousLink =
      id.match(/^u\d+-\d{8}-\d{4}$/) || id.match(/^\d{8}-\d{4}$/);
    ...
    res.json({
      ...
      attendeeEmail:
        slot.confirmed && !isAutonomousLink ? slot.attendeeEmail : null,
      attendeeName:
        slot.confirmed && !isAutonomousLink ? slot.attendeeName : null,
 ...
 */
const handleTimePreferencesTest = async (req: Request, res: Response) => {
  const {
    preference,
    timeSlots,
    slots,
  ...
  if (slots && workingHoursStart && workingHoursEnd) {
    console.log(
      `[TEST] Testing working hours filter in timezone: ${timezone || "UTC"}`
    );
    console.log(
      `[TEST] Working hours: ${workingHoursStart} - ${workingHoursEnd}`
    );
    console.log(`[TEST] Input slots: ${slots.length}`);

    try {
      // Apply the working hours filter with timezone
      const filteredSlots = filterByWorkingHours(
        slots,
        workingHoursStart,
        workingHoursEnd,
        timezone
      ...
      let slotDebugInfo = [];
      if (verbose) {
        slotDebugInfo = slots.map((slot: Slot) => {
          ...
          const isWithinWorkingHours = filteredSlots.some(
  ...
  try {
    const allTimes = timeSlots.map((time) => {
      ...
      const meetsPreference = (timeStr: string, pref: string): boolean => {
        const isAM =
          timeStr.slice(-2) === "am" ||
          (timeStr.includes(":") &&
        ...
        if (pref === "am") return isAM;
        if (pref === "pm") return !isAM;
...
/**
 * Register all API routes for the calendar application
 * @param app Express application instance
 * @returns HTTP server instance
 */
// Export the generateShorterSlotsFromRange function for testing
export { generateShorterSlotsFromRange };
