...
import { preserveSourceTimezoneNull } from "./timezone-response-fix";
...
export const isLocalTimeByTimezone = (
...
) => `Extract exact dates, times, meeting duration, and context from the text. Use ${referenceDate} as the reference date for relative dates (e.g., 'tomorrow,' 'next week').
 ...
 */
export const dateExtractionSchema = {
  name: "email_analysis",
  ...
  parameters: {
    ...
    properties: {
              ...
              items: {
                ...
                properties: {
                  start: {
                    ...
                    type: "string",
                    description:
                      "End time in 24-hour format (HH:MM), or null if not specified.",
                    nullable: true,
                  },
                  isSpecificTime: {
                    type: "boolean",
                    description:
                      "Whether this is a specific time point (at X) vs. a range.",
                  },
                  isTimeRange: {
                    type: "boolean",
                    description:
                      "Whether this is a time range (between X and Y, or from X to Y).",
                  },
                  isAllDay: {
                    type: "boolean",
                  ...
                  sourceTimezone: {
                    type: "string",
                    description:
                      "The timezone mentioned in the email, returned strictly in IANA format (e.g., 'America/New_York', 'Europe/Kyiv'). Do not return abbreviations like 'PST' or 'ET'. If an abbreviation is mentioned, convert it to the correct IANA timezone.",
                    nullable: true,
                  },
                },
                required: [
      ...
      event_title: {
        type: "string",
        description:
          "Suggested title for the event based on the email content.",
      },
      email_reply: {
        type: "string",
        description:
          "Suggested response including a placeholder for the selected slot.",
      },
      meeting_duration: {
        type: "number",
        description:
          "The requested meeting duration in minutes extracted from the email content, if specified. Default to null if not mentioned.",
        nullable: true,
      },
    },
 ...
 */
export const processSuggestedSlots = (
  extractedData: any,
  sourceTimezone: string | null = null, // The timezone mentioned in the email (sender's timezone)
  userTimezone: string = "UTC" // The user's timezone for conversion
): Array<{
  start: string;
  end: string;
  originalStart?: string; // Original time format as extracted from email
  originalEnd?: string; // Original time format as extracted from email
  startSource?: string; // Alternative source time for child slots
  endSource?: string; // Alternative source time for child slots
  fromTimeRange?: boolean; // Indicates slot was generated from a time range
  isTimeRange?: boolean;
  isAllDay?: boolean;
  isSpecificTime?: boolean;
  needsStartTimeCalculation?: boolean;
  needsEndTimeCalculation?: boolean;
  sourceTimezone?: string; // The source timezone (sender's timezone)
}> => {
  let suggestedSlots: Array<{
    start: string;
    end: string;
    originalStart?: string; // Original time format as extracted from email
    originalEnd?: string; // Original time format as extracted from email
    ...
    needsStartTimeCalculation?: boolean;
    needsEndTimeCalculation?: boolean;
    sourceTimezone?: string; // The original timezone from the email
  }> = [];

  if (extractedData && extractedData.dates && extractedData.dates.length > 0) {
    console.log(
      "Processing extracted dates using local time approach:",
      extractedData.dates
    );
    ...
    for (const dateInfo of extractedData.dates) {
      try {
        ...
        if (!dateInfo.times || dateInfo.times.length === 0) {
          console.log(
            `No times specified for date ${date}, creating an all-day slot`
          );
          ...
          suggestedSlots.push({
            ...
            sourceTimezone: preserveSourceTimezoneNull(sourceTimezone),
          });
        }
        // Process explicit times from the email
        else {
          console.log(
            `Found ${dateInfo.times.length} explicit times for date ${date}`
          );
          ...
          for (const timeSlot of dateInfo.times) {
            ...
            if (timeSlot.isTimeRange || timeSlot.isAllDay) {
              ...
              if (timeSlot.start && timeSlot.end) {
                ...
                console.log(
                  `Added time range slot with both times specified (local time):`,
                  {
                    start: startTime,
                    end: endTime,
                    isTimeRange: true,
                ...
                const rawSlotSourceTimezone =
                  timeSlot.sourceTimezone || sourceTimezone;
                // Use the proper IANA timezone identifier for reliable conversions
                const slotSourceTimezone = rawSlotSourceTimezone
                  ? convertTimezoneAbbreviation(rawSlotSourceTimezone)
                  : undefined;

                console.log(
                  `Using source timezone for conversion: ${rawSlotSourceTimezone} → ${slotSourceTimezone}`
                );

                // Check if it's local time using the simplified approach:
                // If no timezone is specified, it's local time
                const isLocalTimeFlag = isLocalTimeByTimezone(
                  timeSlot.sourceTimezone || sourceTimezone
                );
                ...
                if (
                  ...
                  try {
                    ...
                    const startInSourceTZ = moment.tz(
                      `${date}T${timeSlot.start}:00`,
                      slotSourceTimezone
                    );
                    const endInSourceTZ = moment.tz(
                      `${date}T${timeSlot.end}:00`,
                      slotSourceTimezone
                    );
                    ...
                    const userStartFormatted = startInUserTZ.format(
                      "YYYY-MM-DDTHH:mm:ss"
                    );
                    const userEndFormatted = endInUserTZ.format(
                      "YYYY-MM-DDTHH:mm:ss"
                    );

                    console.log(
                      `FIXED: Converted time range from ${slotSourceTimezone} to ${userTimezone}:`,
                      {
                    ...
                    while (currentSlotStart.isBefore(endInUserTZ)) {
                      ...
                      if (slotEnd.isSameOrBefore(endInUserTZ)) {
                        ...
                        const slotEndLocal = slotEnd.format(
                          "YYYY-MM-DDTHH:mm:ss"
                        );
                        ...
                        suggestedSlots.push({
                          ...
                          isSpecificTime: true,
                          sourceTimezone:
                            preserveSourceTimezoneNull(slotSourceTimezone),
                        });

                        slotsGenerated++;
                        console.log(
                          `Generated slot ${slotsGenerated}: ${slotStartUTC} to ${slotEndUTC} (${slotStartLocal} to ${slotEndLocal} ${moment
                            .tz(userTimezone)
                            .zoneAbbr()})`
                        );
                      }
                    ...
                    console.log(
                      `Total slots generated from time range: ${slotsGenerated}`
                    );
                    ...
                    console.error(
                      "Error in improved time range expansion:",
                      conversionError
                    );
              ...
              // CASE 2: Start time is specified but end time is null (e.g., "after 3pm")
                ...
                console.log(
                  `Added time range slot with only start time (local time):`,
                  {
                    start: startTime,
                ...
                suggestedSlots.push({
                  ...
                  isAllDay: false,
                  needsEndTimeCalculation: true, // Flag to indicate end time needs replacement
                  sourceTimezone: preserveSourceTimezoneNull(
                    timeSlot.sourceTimezone || sourceTimezone
                  ), // Store the specific time's timezone with null preserved
                });
              }
              // CASE 3: End time is specified but start time is null (e.g., "before 5pm")
              else if (
                (!timeSlot.start || timeSlot.start === null) &&
                timeSlot.end
              ) {
                const endTime = `${date}T${timeSlot.end}:00`;
                // Use empty string instead of null for TypeScript compatibility
                ...
                console.log(
                  `Added time range slot with only end time (local time):`,
                  {
            ...
            // For specific times, process them as before
              ...
              console.log(
                `Using source timezone for specific time: ${rawSlotSourceTimezone} → ${slotSourceTimezone}`
              );
              ...
              const isLocalTime = isLocalTimeByTimezone(
                timeSlot.sourceTimezone || sourceTimezone
              );
              ...
              if (
                ...
                try {
                  ...
                  console.log(
                    `Converted specific time from ${slotSourceTimezone} to ${userTimezone}:`,
                    {
                  ...
                  console.error(
                    "Error converting timezone for specific time:",
                    conversionError
                  );
              ...
              suggestedSlots.push({
                ...
                originalEnd: timeSlot.end,
                isSpecificTime: true,
                sourceTimezone: preserveSourceTimezoneNull(slotSourceTimezone), // Store the specific time's timezone
              });
            }
          }
        }
