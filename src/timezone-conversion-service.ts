 ...
 */
export function convertTimezoneSafe(
  date: string,
  time: string,
  ...
  try {
    ...
    const sourceIANA = convertTimezoneAbbreviation(sourceTimezone) || "UTC";
    const targetIANA = convertTimezoneAbbreviation(targetTimezone) || "UTC";

    // Step 1: Parse with source timezone
    const dateTimeStr = `${date}T${time}:00`;
    const timeInSourceTz = moment.tz(dateTimeStr, sourceIANA);
...
/**
 * Get timezone abbreviation for a specific IANA timezone
 * Used to provide human-readable timezone indicators in the UI
 *
...
export function getTimezoneAbbreviation(
  ...
  try {
    ...
    const momentObj = date ? moment.tz(date, timezone) : moment.tz(timezone);
 ...
 */
export function convertExtractedTimesToUserTimezone(
  extractedData: any,
  userTimezone: string
): any {
    ...
    if (dateInfo.times && dateInfo.times.length > 0) {
      processedDateInfo.times = dateInfo.times.map((timeSlot: any) => {
        ...
        const sourceIANA = timeSlot.sourceTimezone
          ? convertTimezoneAbbreviation(timeSlot.sourceTimezone)
          : null;
        ...
        if (sourceIANA && sourceIANA !== userTimezone) {
          try {
            ...
            if (startTimeValue) {
              ...
              console.log(
                `Converting start time from ${sourceIANA} to ${userTimezone}`
              );

              // Step 1: Parse with source timezone
              const startInSourceTz = moment.tz(
                `${date}T${startTimeValue}:00`,
                sourceIANA
              );
            ...
            if (endTimeValue) {
              // Ensure we're using the IANA timezone identifier, not the abbreviation
              console.log(
                `Converting end time from ${sourceIANA} to ${userTimezone}`
              );

              // Step 1: Parse with source timezone
              const endInSourceTz = moment.tz(
                `${date}T${endTimeValue}:00`,
                sourceIANA
              );
            ...
            console.log(
              `Converted time from ${sourceIANA} to ${userTimezone}:`,
              {
                original: {
            ...
            console.error(
              `Error converting time from ${sourceIANA} to ${userTimezone}:`,
              error
            );
...
/**
 * Generates candidate meeting slots from the extracted dates and times
 *
 ...
 */
/**
 * Interface for a comprehensively defined time slot with timezone information
 */
export interface Slot {
  // Basic slot information (same as startLocal/endLocal for convenience)
  start: string;
  end: string;

  // Time format variations
  startLocal?: string; // Time in user's local timezone
  endLocal?: string; // Time in user's local timezone
  startUTC?: string; // Time converted to UTC with Z suffix
  endUTC?: string; // Time converted to UTC with Z suffix

  // Source information from the email
  fromTimeRange?: boolean;
  sourceTimezone?: string | null; // IANA timezone from email (e.g., "America/New_York")
  sourceTime?: string | null; // Abbreviation of source timezone (e.g., "EDT")
  startSource?: string | null; // Start time converted back to source timezone
  endSource?: string | null; // End time converted back to source timezone

  // Original values (deprecated, use startSource/endSource instead)
  originalStart?: string | null; // @deprecated
  originalEnd?: string | null; // @deprecated

  // Metadata
  isTimeRange?: boolean;
  isAllDay?: boolean;
  isSpecificTime?: boolean; // Should be true for final slots
  localTimezone?: string; // IANA timezone of user (e.g., "Europe/Kiev")
  localTime?: string; // Abbreviation of user timezone (e.g., "EEST")
  LocalTime?: string; // Capitalized version (will be deprecated)

  // Additional flags used in the app
  fromAllDay?: boolean; // Indicates slot was generated from an all-day event
  adaptedDuration?: number; // Adapted duration if modified from defaults
  needsEndTimeCalculation?: boolean; // For open-ended time ranges
}
...
export function generateSlots(
  extractedData: any,
  userTimezone: string,
  meetingDuration: number | null = 45,
  ...
  const suggestedSlots: Array<Slot> = [];
  ...
  for (const dateInfo of extractedData.dates) {
    ...
    if (!dateInfo.times || dateInfo.times.length === 0) {
      ...
      suggestedSlots.push({
        ...
        endUTC:
          moment.tz(endTime, userTimezone).utc().format("YYYY-MM-DDTHH:mm:ss") +
          "Z",
      ...
      for (const timeSlot of dateInfo.times) {
        ...
        if (!timeSlot.convertedStart && !timeSlot.originalStart) continue;
        ...
        if (timeSlot.isTimeRange || timeSlot.isAllDay) {
          ...
          const startTimeValue =
            timeSlot.convertedStart || timeSlot.originalStart;
          ...
          if (startTimeValue && endTimeValue) {
            ...
            const startMoment = moment.tz(startTime, userTimezone);
            const endMoment = moment.tz(endTime, userTimezone);
            ...
            while (currentSlotStart.isBefore(endMoment)) {
              ...
              if (slotEnd.isSameOrBefore(endMoment)) {
                ...
                const slotStartLocal = currentSlotStart.format(
                  "YYYY-MM-DDTHH:mm:ss"
                );
                ...
                const slotStartUTC =
                  currentSlotStart.clone().utc().format("YYYY-MM-DDTHH:mm:ss") +
                  "Z";
                const slotEndUTC =
                  slotEnd.clone().utc().format("YYYY-MM-DDTHH:mm:ss") + "Z";
                ...
                const actualSourceTimezone =
                  timeSlot.sourceTimezone || userTimezone;
                ...
                if (actualSourceTimezone !== userTimezone) {
                  try {
                    ...
                    const localStartMoment = moment.tz(
                      slotStartLocal,
                      userTimezone
                    );
                    ...
                    const localEndMoment = moment.tz(
                      slotEndLocal,
                      userTimezone
                    );
                    ...
                    console.error(
                      "Error converting time back to source timezone:",
                      error
                    );
                    originalStart =
            ...
            if (actualSourceTimezone !== userTimezone) {
              try {
                // Convert current slot time from local to source timezone
                const localStartMoment = moment.tz(startTime, userTimezone);
                const sourceStartMoment = localStartMoment
                  .clone()
                  .tz(actualSourceTimezone);
                originalStart = sourceStartMoment.format("HH:mm");

                const localEndMoment = moment.tz(endTime, userTimezone);
                const sourceEndMoment = localEndMoment
                  .clone()
                  .tz(actualSourceTimezone);
                originalEnd = sourceEndMoment.format("HH:mm");
          ...
          suggestedSlots.push({
            ...
            startUTC:
              moment
                ...
                .format("YYYY-MM-DDTHH:mm:ss") + "Z",
            endUTC:
              endTimeMoment.clone().utc().format("YYYY-MM-DDTHH:mm:ss") + "Z",
 ...
 */
export function processEmailAnalysisWithTimezones(
  extractedData: any,
  userTimezone: string,
  meetingDuration?: number | null,
  slotInterval?: number
): { extractedData: any; candidateSlots: Array<Slot> } {
  ...
  if (
    purifiedExtractedData &&
    purifiedExtractedData.dates &&
    purifiedExtractedData.dates.length > 0
  ) {
    purifiedExtractedData.dates.forEach((date: any) => {
      if (date.times && date.times.length > 0) {
        date.times.forEach((timeSlot: any) => {
  ...
  candidateSlots.forEach((slot) => {
    ...
    let sourceIANA: string | null = null;
    if (slot.sourceTimezone) {
      // When getting IANA timezone, always use the conversion function first
      sourceIANA = convertTimezoneAbbreviation(slot.sourceTimezone);
      console.log(
        `Source timezone ${slot.sourceTimezone} mapped to IANA: ${sourceIANA}`
      );
      ...
      if (sourceIANA) {
        try {
          ...
          const sourceTzMoment = moment.tz(dateTimeForDst, sourceIANA);
          ...
          console.log(
            `Set sourceTime to ${slot.sourceTime} based on date ${slotDate}`
          );
        } catch (error) {
          console.error(
            `Error getting timezone abbreviation for ${sourceIANA}:`,
            error
          );
    ...
    if (sourceIANA && slot.startLocal) {
      try {
        ...
        const startConversion = convertTimezoneSafe(
          dateStr,
          timeStr,
          userTimezone,
          sourceIANA
      ...
      if (slot.endLocal) {
        try {
          ...
          // Use convertTimezoneSafe to convert from user timezone to source timezone
          const endConversion = convertTimezoneSafe(
            dateStr,
            timeStr,
            userTimezone,
            sourceIANA
    ...
    if ("LocalTime" in slot) {
      const slotWithLocalTime = slot as any;
 ...
 */
export function generateShorterSlotsFromRange(
  slot: Slot,
  ...
  console.log(
    "[SLOT_GENERATION:DETAIL] Starting slot generation with parameters:"
  );
  console.log(
    `[SLOT_GENERATION:DETAIL] Meeting duration: ${meetingDuration} minutes`
  );
  console.log(
    `[SLOT_GENERATION:DETAIL] Slot interval: ${slotInterval} minutes`
  );
  console.log(
    `[SLOT_GENERATION:DETAIL] User timezone: ${userTimezone || "Not provided"}`
  );
  console.log(`[SLOT_GENERATION:DETAIL] Input slot: ${JSON.stringify(slot)}`);
  // Log input slot flags for debugging
  console.log(
    `[SLOT_GENERATION:DETAIL] Input slot flags: isTimeRange=${
      slot.isTimeRange
    }, isSpecificTime=${slot.isSpecificTime}, fromTimeRange=${
      slot.fromTimeRange
    }, sourceTimezone=${slot.sourceTimezone || "null"}`
  );

  if (!slot.start || !slot.end) {
    console.log(
      "[SLOT_GENERATION:ERROR] Cannot generate slots from incomplete time range"
    );
  ...
  try {
    ...
    const isLocalTime =
      slot.sourceTimezone === null || slot.sourceTimezone === undefined;
    ...
    if (isLocalTime && userTimezone) {
      ...
      console.log(
        `[SLOT_GENERATION:DETAIL] Processing local time slot in timezone ${userTimezone}`
      );
      ...
      rangeStart = moment.tz(`${startDate}T${startTime}`, userTimezone);
      rangeEnd = moment.tz(`${endDate}T${endTime}`, userTimezone);
      ...
      console.log(
        `[SLOT_GENERATION:DETAIL] Converting from ${slot.sourceTimezone} to ${userTimezone}`
      );

      // Create moments in the source timezone
      rangeStart = moment.tz(slot.start, slot.sourceTimezone);
      rangeEnd = moment.tz(slot.end, slot.sourceTimezone);
      ...
      console.log(`[SLOT_GENERATION:DETAIL] Converted times: 
        Start in user timezone: ${rangeStart.format(
          "YYYY-MM-DD HH:mm"
        )} ${userTimezone}
        End in user timezone: ${rangeEnd.format(
          "YYYY-MM-DD HH:mm"
        )} ${userTimezone}`);
    } else {
      // For UTC time slots with no timezone info, just parse them normally
      rangeStart = moment(slot.start);
      rangeEnd = moment(slot.end);
    ...
    if (
      ...
      console.log(
        "[SLOT_GENERATION:ERROR] Invalid time range - start is same or after end time"
      );
      return [slot];
    }
    ...
    console.log(
      `[SLOT_GENERATION:DETAIL] Processing slot with duration ${durationMinutes} minutes, ` +
    ...
    const isEmailTimeRange =
      slot.isTimeRange === true || slot.fromTimeRange === true;
    ...
    if (durationMinutes < meetingDuration) {
      ...
      if (isEmailTimeRange) {
        console.log(
          `[SLOT_GENERATION:ADAPT] Respecting email-specified time range (${durationMinutes} min) even though shorter than preferred duration (${meetingDuration} min)`
        );
      ...
      // Otherwise, only adapt if the duration is at least 25 minutes
        ...
        console.log(
          `[SLOT_GENERATION:ADAPT] Adapting meeting duration from ${meetingDuration} to ${durationMinutes} minutes to fit the shorter time range`
        );
        adaptedMeetingDuration = durationMinutes;
        isAdaptedDuration = true;
      } else {
        ...
        console.log(
          `[SLOT_GENERATION:ERROR] Time range (${durationMinutes} min) is too short (< 25 min) for a meeting`
        );
        ...
        if (isEmailTimeRange && durationMinutes >= 10) {
          console.log(
            `[SLOT_GENERATION:RECOVER] Even though very short, keeping original email time range as a single slot`
          );
          ...
          return [
            {
              ...
              localTimezone:
                localTimezone || slot.localTimezone || userTimezone || "UTC",
            },
          ];
        }
    ...
    while (
      ...
      console.log(
        `[SLOT_GENERATION:DETAIL] Generated slot ${
          slotCount + 1
        }: ${localStart} - ${localEnd} in ${userTimezone || "UTC"}`
      );
    ...
    if (!enteredLoop) {
      console.log(
        `[SLOT_GENERATION:ERROR] Loop condition failed: first slot wouldn't end before range end`
      );
      console.log(
        `[SLOT_GENERATION:ERROR] First potential end time: ${rangeStart
          .clone()
          .add(meetingDuration, "minutes")
          .format("YYYY-MM-DD HH:mm")}`
      );
      console.log(
        `[SLOT_GENERATION:ERROR] Range end: ${rangeEnd.format(
          "YYYY-MM-DD HH:mm"
        )}`
      );
      console.log(
        `[SLOT_GENERATION:ERROR] Comparison: ${rangeStart
          .clone()
          .add(meetingDuration, "minutes")
          .isSameOrBefore(rangeEnd)}`
      );
      ...
      if (isEmailTimeRange || durationMinutes >= 25) {
        ...
        console.log(
          "[SLOT_GENERATION:RECOVERY] Creating at least one slot that fits within the range"
        );
        ...
        const finalDuration =
          isEmailTimeRange && useDuration < 10 ? 10 : useDuration;

        console.log(
          `[SLOT_GENERATION:RECOVERY] Using duration from ${
            isEmailTimeRange ? "email analysis" : "user preferences"
          }: ${finalDuration} minutes`
        );

        console.log(
          `[SLOT_GENERATION:RECOVERY] Using ${finalDuration} minutes for slot duration`
        );
        ...
        if (slot.sourceTimezone) {
          ...
          const slotSourceTimezone =
            convertTimezoneAbbreviation(safeTz) || safeTz;

          // Source time (original timezone)
          const startSource =
            slot.originalStart || slot.startSource || slotStartISO;
          ...
          startUTC =
            moment.utc(slotStartISO).format("YYYY-MM-DDTHH:mm:ss") + "Z";
          endUTC = moment.utc(slotEndISO).format("YYYY-MM-DDTHH:mm:ss") + "Z";
          startLocal = slotStartISO;
          endLocal = slotEndISO;
        }
        ...
        generatedSlots.push({
          ...
          // Additional metadata
          adaptedDuration:
            useDuration !== meetingDuration ? useDuration : undefined,
          originalStart: slot.originalStart,
          originalEnd: slot.originalEnd,
        });

        console.log(
          `[SLOT_GENERATION:RECOVERY] Added one recovery slot: ${slotStartISO} - ${slotEndISO}`
        );
        console.log(
          `[SLOT_GENERATION:RECOVERY] Slot flags: isTimeRange=false, isSpecificTime=true, fromTimeRange=${
            slot.isTimeRange === true ? true : slot.fromTimeRange || false
          }`
        );
      }
    }
    ...
    if (slotCount >= maxSlots) {
      console.warn(
        "[SLOT_GENERATION:WARNING] Maximum slot limit reached, may indicate an issue with input data"
      );
    }
    ...
    if (generatedSlots.length === 0) {
      console.log(
        "[SLOT_GENERATION:ERROR] No slots were generated, returning original slot"
      );
      return [slot];
    }

    console.log(
      `[SLOT_GENERATION:SUCCESS] Generated ${generatedSlots.length} slots from time range ${slot.start} - ${slot.end}`
    );

    // Log a sample of generated slots
    generatedSlots.slice(0, 3).forEach((genSlot, idx) => {
      console.log(
        `[SLOT_GENERATION:DETAIL] Sample slot ${idx + 1}: ${genSlot.start} - ${
          genSlot.end
        }, flags: isTimeRange=${genSlot.isTimeRange}, isSpecificTime=${
          genSlot.isSpecificTime
        }, fromTimeRange=${genSlot.fromTimeRange}, sourceTimezone: ${
          genSlot.sourceTimezone || "local"
        }`
      );
    });
    if (generatedSlots.length > 3) {
      console.log(
        `[SLOT_GENERATION:DETAIL] ... and ${
          generatedSlots.length - 3
        } more slots`
      );
    }
  } catch (error) {
    console.error(
      "[SLOT_GENERATION:ERROR] Error generating slots from time range:",
      error
    );
